...
public class GestionJSONAbonnement {
	 ...
	 */
	public static List<CategorieAbonnement> lireJSON(String fichierJSON) {
	    try {
	        JsonReader reader = new JsonReader(new FileReader(fichierJSON));
	        // On peut lire un seul objet ou un tableau avec []
	        CategorieAbonnement[] tabCat = new Gson().fromJson(reader, CategorieAbonnement[].class);
	        return Arrays.asList(tabCat);
	    } catch (FileNotFoundException e) {
	        System.err.println("Erreur: Le fichier JSON n'a pas été trouvé - " + e.getMessage());
	    } catch (JsonParseException e) {
	        System.err.println("Erreur: Échec de la lecture du fichier JSON - " + e.getMessage());
	    } catch (Exception e) {
	        System.err.println("Erreur: Une exception s'est produite - " + e.getMessage());
	    }
	    return new ArrayList<>(); // Retourne une liste vide si une erreur se produit
	}
	 ...
	 */
	public static boolean EcrireJsonManuelle(String fichierJSON, List<CategorieAbonnement> lstCat) {
		try {
			JsonWriter writer = new JsonWriter(new FileWriter(fichierJSON));
			writer.beginArray(); // une liste devient un tableau en JSON
			for (CategorieAbonnement cat : lstCat) {
				writer.beginObject();
				writer.name("name").value(cat.getName()); // champs unique
				writer.name("lstPdt");
				writer.beginArray(); // tableau des produits
				for (Abonnement pdt : cat.getLstPdt()) {
					writer.beginObject();
					writer.name("dateAbonnement").value(pdt.getDateAbonnement());
					writer.name("dateFinAbonnement").value(pdt.getDateFinAbonnement());
					writer.endObject();
				}
				writer.endArray();
				writer.endObject();

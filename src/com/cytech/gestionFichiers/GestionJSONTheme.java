...
public class GestionJSONTheme {
	 ...
	 */
	public static List<CategorieTheme> lireJSON(String fichierJSON) {
		try {
			JsonReader reader = new JsonReader(new FileReader(fichierJSON));
			// On peut lire un seul objet ou un tableau avec []
			CategorieTheme[] tabCat = new Gson().fromJson(reader, CategorieTheme[].class);

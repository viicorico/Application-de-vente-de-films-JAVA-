...
public class GestionJSONPanier {
	 ...
	 */
	public static List<CategoriePanier> lireJSON(String fichierJSON) {
		try {
			JsonReader reader = new JsonReader(new FileReader(fichierJSON));
			// On peut lire un seul objet ou un tableau avec []
			CategoriePanier[] tabCat = new Gson().fromJson(reader, CategoriePanier[].class);
	 ...
	 */
	public static boolean EcrireJsonManuelle(String fichierJSON, List<CategoriePanier> lstCat) {
		try {
			JsonWriter writer = new JsonWriter(new FileWriter(fichierJSON));
			writer.beginArray(); // une liste devient un tableau en JSON
			for (CategoriePanier cat : lstCat) {
				writer.beginObject();
				writer.name("name").value(cat.getName()); // champs unique
				writer.name("lstpdt	");
				writer.beginArray(); // tableau des produits
				for (Panier pdt : cat.getLstPdt()) {
					writer.beginObject();
					writer.name("prixAPayer").value(pdt.getPrixAPayer());
					writer.name("tvaAppliquee").value(pdt.getTvaAppliquee());
					writer.endObject();
				}
				writer.endArray();
				writer.endObject();

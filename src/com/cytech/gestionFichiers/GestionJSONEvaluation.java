...
public class GestionJSONEvaluation {
	 ...
	 */
	public static List<CategorieEvaluation> lireJSON(String fichierJSON) {
		try {
			JsonReader reader = new JsonReader(new FileReader(fichierJSON));
			// On peut lire un seul objet ou un tableau avec []
			CategorieEvaluation[] tabCat = new Gson().fromJson(reader, CategorieEvaluation[].class);
	 ...
	 */
	public static boolean EcrireJsonManuelle(String fichierJSON, List<CategorieEvaluation> lstCat) {
		try {
			JsonWriter writer = new JsonWriter(new FileWriter(fichierJSON));
			writer.beginArray(); // une liste devient un tableau en JSON
			for (CategorieEvaluation cat : lstCat) {
				writer.beginObject();
				writer.name("name").value(cat.getName()); // champs unique
				writer.name("lstpdt	");
				writer.beginArray(); // tableau des produits
				for (Evaluation pdt : cat.getLstPdt()) {
					writer.beginObject();
					writer.name("note").value(pdt.getNote());
					writer.name("commentaire").value(pdt.getCommentaire());
					writer.name("date").value(pdt.getDate());
					writer.name("active").value(pdt.getActive());
					writer.endObject();
				}
				writer.endArray();
				writer.endObject();

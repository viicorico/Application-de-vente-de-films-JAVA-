...
public class GestionJSONAchat{
	
	 
	 
	 
	 public static List<CategorieAchat> lireJSON(String fichierJSON) {
			try {
				JsonReader reader = new JsonReader(new FileReader(fichierJSON));
				// On peut lire un seul objet ou un tableau avec []
				CategorieAchat[] tabCat = new Gson().fromJson(reader, CategorieAchat[].class);
		 ...
		 */
		public static boolean EcrireJsonManuelle(String fichierJSON, List<CategorieAchat> lstCat) {
	        if (lstCat == null) {
	            System.out.println("La liste des catégories d'achat est null.");
	            return false;
	        }
	        
	        try (JsonWriter writer = new JsonWriter(new BufferedWriter(new FileWriter(fichierJSON)))) {
	            writer.beginArray(); // une liste devient un tableau en JSON
	            
	            for (CategorieAchat cat : lstCat) {
	                writer.beginObject();
	                writer.name("name").value(cat.getName()); // champs unique
	                writer.name("listeAchat");
	                
	                if (cat.getlisteAchat() != null) {
	                    writer.beginArray(); // tableau des Achats
	                    
	                    for (Achat pdt : cat.getlisteAchat()) {
	                        writer.beginObject();
	                        writer.name("achatID").value(pdt.getAchatID());
	                        writer.name("montant").value(pdt.getMontant());
	                        writer.name("facture").value(pdt.getFacture());
	                        writer.name("date").value(pdt.getDate());
	                        // Écrire les attributs client et listeFilm même s'ils sont null
	                        writer.name("client").beginObject().endObject();
	                        writer.name("listeFilm").beginArray().endArray();
	                        writer.endObject();
	                    }
	            ...
	            System.out.println("Erreur d'écriture du fichier JSON : " + e.getMessage());
	            return false;
	        }
	    }
}
